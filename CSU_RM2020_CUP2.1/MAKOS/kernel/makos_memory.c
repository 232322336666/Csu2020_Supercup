...
//内存池(8字节对齐)
__align(8) u8 mem1base[MEM1_MAX_SIZE] ;																					//内部SRAM内存池
__align(8) u8 mem2base[MEM2_MAX_SIZE] __attribute__((at(0X68000000)));					//外部SRAM内存池
...
{  
    ...
    if(size%memblksize[memx])xmemb++;//尾部余出包含  
    ...
    {     
			...
			if(kmemb==xmemb)							//找到了连续nmemb个空内存块
			{
							...
							return (offset*memblksize[memx]);//返回偏移地址,相对于内部ram起始地址或者外部ram起始地址  
			}
		}  
